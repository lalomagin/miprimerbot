...
app.post("/apinooficial", async (req, res) => {
    let respuesta = { status: "-1", message: "No inicializado" };
    const body = req.body;
    ...
    try {
        switch (body.op) {
            case "reiniciarserver":
                const respuesta_iniciar = await new Promise((resolve) => {
                    exec(tarea_iniciar, (error, stdout, stderr) => {
                ...
                log("reiniciarserver:response", { respuesta_iniciar });
                respuesta = { status: "0", message: "Se reinicio server" };
                break;

           
            
            case "reiniciarserveryiniciarqr": 

                if (clientDefault) {
                    console.log(" Cerrando sesión anterior de WhatsApp...");
                    await clientDefault.destroy(); // Cierra la sesión de WhatsApp
                ...
                const respuesta_iniciar1 = await new Promise((resolve) => {
                    exec(tarea_iniciar, (error, stdout, stderr) => {
                ...
                log("reiniciarserver:response", { respuesta_iniciar1 });
                //respuesta = { status: "0", message: "Se reinicio server" };
                ...
                respuesta = clientDefault ? { status: "0", message: "Se esta iniciando en el Web Whatsapp, espero de 10 a 40 seg a que se genere el QR y luego escanealo" } : { status: "1", message: "No se puede generar iniciar el Web Whatsapp" };
